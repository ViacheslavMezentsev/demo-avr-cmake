...
bool FatFsCard::begin( uint8_t chipSelectPin, uint8_t sckDivisor )
{
	...
	auto t0 = ( uint16_t ) millis();
	uint32_t arg;
	//uint8_t csPin52 = 52;

	// initialize SPI for SD card
	spi.Initialize( spiUp, spiDown );
	...
	//if (csPin52 == m_CSPin) csPin52 = 4;	

	//SPI.beginTransaction( csPin52, SPISettings( m_spifrec, MSBFIRST, SPI_MODE0 ));
	//SPI.beginTransaction(SPISettings(m_spifrec, MSBFIRST, SPI_MODE0));	

	// Dummy clocks.
	for ( uint8_t i = 0; i < 10; i++ ) spi.Read();
	...
	if ( type() == SD_CARD_TYPE_SD2 )
	{
		if ( cardCommand( CMD58, 0 ) )
		{
			...
			printf_P( F( "SD_CARD_ERROR_CMD58\n" ) );
