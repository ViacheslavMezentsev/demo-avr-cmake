...
auto is_close_fraction( const FloatingPointType a, const FloatingPointType b,
...
auto cyl_bessel_j( const std::uint_fast8_t n, const FloatingPointType& x ) noexcept -> FloatingPointType
{
    ...
    const auto jn = static_cast<floating_point_type>( integration_result / pi_v<floating_point_type> );
...
auto main() -> int
{
    ...
    constexpr my_float_type my_tol = static_cast<my_float_type>( std::numeric_limits<my_float_type>::epsilon() * static_cast<my_float_type>( 100.0L ) );

    // Compute y = cyl_bessel_j(2, 1.23) = 0.16636938378681407351267852431513159437103348245333
    // N[BesselJ[2, 123/100], 50]
    const my_float_type j2 = cyl_bessel_j( UINT8_C(2), static_cast<my_float_type>( 1.23L ) );
